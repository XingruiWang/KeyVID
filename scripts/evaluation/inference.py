...
def load_data_prompts(data_dir, video_size=(256,256), video_frames=16, interp=False):
    ...
    ## load video
    file_list = get_filelist(data_dir, ['jpg', 'png', 'jpeg', 'JPEG', 'PNG', 'mp4'])
    print(file_list)
    ...
    for idx in range(n_samples):
        ...
        if interp:
            ...
            if file_list[idx].endswith('mp4'):
                
                video_reader = VideoReader(file_list[idx], ctx=cpu(0))
                frames = video_reader.get_batch([0])
                image = Image.fromarray(frames.asnumpy()[0])
...
def image_guided_synthesis(model, prompts, videos, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
    ...
    # What's this two modules?
    # Image plays two roles: prompt (by cross attention) and condition -> no temporal contraint, repeat through frames
    # One idea: align audio sequences with time frame?
    # Read the code, how to encode the audio
    img = videos[:,:,0] #bchw
    ...
    cond["c_audio"]=torch.zeros(img.size(0), 16, 19, 768)
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args.bs)), desc='Sample Batch'):
            ...
            for nn, samples in enumerate(batch_samples):
                ...
                save_results_seperate(prompt, samples, filename, fakedir, fps=args.frame_stride, loop=args.loop)
