...
class AudioVideoAlignedMultiPairDataset(Dataset):
	...
	def __init__(
		...
		self.load_path(example_list_path)
	
	def load_path(self, example_list_path):
		with open(example_list_path, "r") as f:
			examples = f.readlines()
		exist_path = []
		for example in examples:
			processed_example = '_'.join(example.strip().split("/")[-1].split("_")[:-1])+'.mp4'
			if not osp.exists(osp.join(self.data_root, processed_example)):
				continue
			else:
				exist_path.append(processed_example)
...
if __name__ == "__main__":
	...
	for i in range(1000):
		...
		print(data["video"].min(), data["video"].max())
		print(data["audio"].min(), data["audio"].max())
		print(data["video"].mean(), data["video"].std())
		print(data["audio"].mean(), data["audio"].std())
		print()