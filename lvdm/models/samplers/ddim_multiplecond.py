...
class DDIMSampler(object):
    ...
    def sample(self,
               ...
               mask=None,
               x0=None,
               x_keyframe=None, keyframe_idx = None, end_step = 0,
               temperature=1.,
               noise_dropout=0.,
        ...
        samples, intermediates = self.ddim_sampling(conditioning, size,
                                                    ...
                                                    mask=mask, x0=x0,
                                                    x_keyframe=x_keyframe, keyframe_idx = keyframe_idx,
                                                    end_step=end_step,
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      mask=None, x0=None, 
                      x_keyframe=None, keyframe_idx = None,
                      end_step = 0,
        ...
        for i, step in enumerate(iterator):
            ...
            if x_keyframe is not None:
                assert keyframe_idx is not None
                assert not clean_cond
                if clean_cond:
                    img_orig = x_keyframe
                else:
                    img_orig = self.model.q_sample(x_keyframe, ts)  # TODO: deterministic forward pass? <ddim inversion>
                
                # img = img_orig * mask + (1. - mask) * img # keep original & modify use img
                # weight schedule !!!
                # initialization nosise / step wise viaulization
                # img[:, :, keyframe_idx[0].long()] = img_orig.clone() * 0.8 + img[:, :, keyframe_idx[0].long()] * 0.2 # not use
                # img[:, :, keyframe_idx[0].long()] = img_orig.clone() 

            # this is for inpainting trial
            if i > end_step:
                x_keyframe = None
                # keyframe_idx = None
            ...
            #                             mask=mask,x0=x0,fs=fs,guidance_rescale=guidance_rescale,
            #                             **kwargs)
            #     img_tmp, pred_x0 = outs

            #     img = self.model.q_sample(img_tmp, ts)

            
            if callback: callback(i)
            if img_callback: img_callback(pred_x0, i)

            if index % 2 == 0 or index == total_steps - 1:
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      ...
                      uc_type=None, cfg_img=None,cfg_audio=None,mask=None,x0=None,guidance_rescale=0.0, 
                      x_keyframe=None, keyframe_idx = None,**kwargs):
        ...
        unconditional_conditioning_img_text_noneaudio = kwargs['unconditional_conditioning_img_text_noneaudio']
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            e_t_uncond_img_text = self.model.apply_model(x, t, unconditional_conditioning_img_text_noneaudio, **kwargs)
            ...
            model_output = e_t_uncond + cfg_img * (e_t_uncond_img - e_t_uncond) + unconditional_guidance_scale * (e_t_uncond_img_text - e_t_uncond_img) + cfg_audio * (e_t_cond - e_t_uncond_img_text)
        ...
        if x_keyframe is not None:
            # here            
            x_keyframe_known = a_prev.sqrt() * x_keyframe.clone() + (1 - a_prev).sqrt() * noise[:, :, keyframe_idx[0].long()] / sigma_t 
            x_prev[:, :, keyframe_idx[0].long()] = x_keyframe_known
