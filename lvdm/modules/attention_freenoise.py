...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None, context_next=None, context_keyframes = None, use_injection=False, position=None):
        assert position is None
        spatial_self_attn = (context is None)
        
        # sa_flag = spatial_self_attn
        all_k_ip, all_v_ip, out_ip = None, None, None
        all_k_ap, all_v_ap, out_ap = None, None, None
        ...
        if self.image_cross_attention and self.audio_cross_attention and not spatial_self_attn:
            ...
            all_k_ip = self.to_k_ip(context_image)
            all_v_ip = self.to_v_ip(context_image)
            all_k_ap = self.to_k_ap(context_audio)
            all_v_ap = self.to_v_ap(context_audio)
        ...
        if spatial_self_attn and context_next is not None:
            all_q, all_k, all_v = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=h), (all_q, all_k, all_v))

            if all_k_ip is not None:
                all_k_ip, all_v_ip = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=h), (all_k_ip, all_v_ip))
            if all_k_ap is not None:
                all_k_ap, all_v_ap = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=h), (all_k_ap, all_v_ap))
            ...
            for t_start, t_end in context_next:
                ...
                if k_ap is not None:
                    ...
                    k_ap = all_k_ap[:, t_start:t_end]
                    v_ap = all_v_ap[:, t_start:t_end]
            ...
            if out_ip is not None:
                if self.image_cross_attention_scale_learnable:
                    final_out = final_out + self.image_cross_attention_scale * out_ip * (torch.tanh(self.alpha)+1)
                else:
                    final_out = final_out + self.image_cross_attention_scale * out_ip
            
            if all_k_ap is not None:
                k_ap, v_ap = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=h), (all_k_ap, all_v_ap))
                sim_ap =  torch.einsum('b i d, b j d -> b i j', q, k_ap) * self.scale
                del k_ap
                sim_ap = sim_ap.softmax(dim=-1)
                out_ap = torch.einsum('b i j, b j d -> b i d', sim_ap, v_ap)
            ...
            if out_ap is not None:
                if self.audio_cross_attention_scale_learnable:
                    final_out = final_out + self.audio_cross_attention_scale * out_ap * (torch.tanh(self.beta)+1)
                else:
                    final_out = final_out + self.audio_cross_attention_scale * out_ap
    ...
    def efficient_forward_FA(self, x, context=None, mask=None, context_next=None, use_injection=False, position=None):
        assert position is None
        spatial_self_attn = (context is None)
        ...
        if spatial_self_attn and context_next is not None:
            ...
            if context_keyframes is not None:
                weight_sequence = generate_weight_sequence(len(context_keyframes))
                weight_tensor = torch.ones_like(count[:, context_keyframes])
                weight_tensor = weight_tensor * torch.Tensor(weight_sequence).to(x.device).unsqueeze(0).unsqueeze(-1)

                q = all_q[:, context_keyframes]
                k = all_k[:, context_keyframes]
                v = all_v[:, context_keyframes]
                ...
                ## for image cross-attention
                if k_ip is not None:
                    k_ip = all_k_ip[:, context_keyframes]
                    v_ip = all_v_ip[:, context_keyframes]
                ...
                if k_ap is not None:
                    # k_ap, v_ap = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=h), (k_ap, v_ap))
                    k_ap = all_k_ap[:, context_keyframes]
                    v_ap = all_v_ap[:, context_keyframes]
                ...
                del q
                value[:,context_keyframes] += out * weight_tensor
                count[:,context_keyframes] += weight_tensor
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, n_heads, d_head, dropout=0., context_dim=None, gated_ff=True, checkpoint=True,
        ...
        self.attn2 = attn_cls(query_dim=dim, context_dim=context_dim, heads=n_heads, dim_head=d_head, \
                                ...
                                verbose=verbose, injection=injection)
        self.image_cross_attention = image_cross_attention
    ...
    def forward(self, x, context=None, mask=None, context_next=None, context_keyframes=None, use_injection=False, **kwargs):
        ...
        if "position" in kwargs:
            ...
            input_tuple = (x, context, mask, context_next, context_keyframes, use_injection, position)

        if mask is not None:
            forward_mask = partial(self._forward, mask=mask)
            print(" forward_mask")
            return checkpoint(forward_mask, (x,), self.parameters(), self.checkpoint)
        input_tuple = (x, context, mask, context_next, context_keyframes, use_injection)
    ...
    #     return x
    def _forward(self, x, context=None, mask=None, context_next=None, context_keyframes=None, use_injection=False):
        x = self.attn1(self.norm1(x), context=context if self.disable_self_attn else None, mask=mask, context_next=context_next, context_keyframes=context_keyframes, use_injection=False) + x
        x = self.attn2(self.norm2(x), context=context, mask=mask, context_next=context_next, context_keyframes=context_keyframes, use_injection=use_injection) + x
        x = self.ff(self.norm3(x)) + x
        return x
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head, depth=1, dropout=0., context_dim=None,
                 ...
                 audio_cross_attention=False, audio_cross_attention_scale_learnable=False, injection=False):
        super().__init__()
...
class TemporalTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head, depth=1, dropout=0., context_dim=None,
                 ...
                 relative_position=False, temporal_length=None, full_temporal_length=None, injection=False):
        super().__init__()
        ...
        self.transformer_blocks = nn.ModuleList([
            BasicTransformerBlock(
                ...
                context_dim=context_dim,
                attention_cls=attention_cls,
                checkpoint=use_checkpoint, injection=injection) for d in range(depth)
        ])
        if not use_linear:
    ...
    def forward(self, x, context=None, **kwargs):
        ...
        if not self.full_sequencial_relative_position:
            ...
            for i, block in enumerate(self.transformer_blocks):
                x = block(x, mask=mask, position=position,  **kwargs)
            x = rearrange(x, '(b hw) t c -> b hw t c', b=b).contiguous()
...
class AudioSpatialTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head, depth=1, dropout=0., context_dim=None,
                 use_checkpoint=True, disable_self_attn=False, use_linear=False, video_length=None,
                 image_cross_attention=False, image_cross_attention_scale_learnable=False,
                 audio_context_len=8, injection=False):
