...
from lvdm.modules.attention_freenoise import SpatialTransformer, TemporalTransformer, AudioTemporalTransformer, AudioSpatialTransformer
        ...
        for layer in self:
            ...
            #     x = layer(x, context)
                ...
                if c_audio is not None:
                    ...
                    context = torch.cat([context, c_audio], dim=1)
                x = layer(x, context)
...
class UNetModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.add_frame_idx = add_frame_idx
        ...
        if add_frame_idx:
            self.frame_idx_embedding = nn.Sequential(
                linear(model_channels, time_embed_dim),
                nn.SiLU(),
                linear(time_embed_dim, time_embed_dim),
            )
            nn.init.zeros_(self.frame_idx_embedding[-1].weight)
            nn.init.zeros_(self.frame_idx_embedding[-1].bias)

        if use_qformer:
            # num_query_token = self.temporal_length * 8
            # self.audio_proj_qformer = AudioProjection()
            # self.audio_context_len = 8
            self.audio_context_len = None
        else:
                ...
                if ds in attention_resolutions:
                    ...
                    layers.append(
                        SpatialTransformer(ch, num_heads, dim_head, 
                            ...
                            audio_cross_attention=self.audio_cross_attention,
                            audio_cross_attention_scale_learnable=self.audio_cross_attention_scale_learnable                     
                        )
                    )     
        ...
        layers = [
            ResBlock(ch, time_embed_dim, dropout,
                ...
                use_scale_shift_norm=use_scale_shift_norm, tempspatial_aware=tempspatial_aware,
                use_temporal_conv=temporal_conv,use_swin_block=self.use_swin_block,
            ),
            SpatialTransformer(ch, num_heads, dim_head, 
                depth=transformer_depth, context_dim=context_dim, use_linear=use_linear,
    ...
    def forward(self, x, timesteps, context=None, c_audio = None, features_adapter=None, fs=None, frame_idx = None, **kwargs):
        ...
        if self.add_frame_idx:            
            b, t_f = frame_idx.shape
            frame_idx_list = []
            for t in range(t_f):
                frame_idx_emb = timestep_embedding(frame_idx[:, t], self.model_channels, repeat_only=False).type(x.dtype)
                frame_idx_embed = self.frame_idx_embedding(frame_idx_emb)
                frame_idx_list.append(frame_idx_embed)
            frame_idx_embed = torch.stack(frame_idx_list, dim=1)
            frame_idx_embed = rearrange(frame_idx_embed, 'b t c -> (b t) c')
            emb = emb + frame_idx_embed
