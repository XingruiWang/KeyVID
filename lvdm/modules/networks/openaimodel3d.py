...
#             )
#         self.audio_proj = nn.Linear(self.Qformer.config.hidden_size, 1024)
...
#         c_audio = rearrange(c_audio, 'b (t l) c -> (b t) l c', l=8) # 2*t, 8, 1024]
#         return c_audio
...
class UNetModel(nn.Module):
            ...
            for _ in range(num_res_blocks):
                ...
                if ds in attention_resolutions:
                    ...
                    if self.temporal_attention:
                        ...
                        # layers.append(
                        #     TemporalSwinTransformer(ch, num_heads, dim_head,
                        #         depth=transformer_depth, context_dim=context_dim, use_linear=use_linear,
                        #         use_checkpoint=use_checkpoint, only_self_att=temporal_self_att_only, 
                        #         causal_attention=use_causal_attention, relative_position=use_relative_position, 
                        #         temporal_length=temporal_length,full_temporal_length=full_temporal_length
    ...
    def forward(self, x, timesteps, context=None, c_audio = None, features_adapter=None, fs=None, frame_idx = None, **kwargs):
        ...
        if l_context == 77 + t*16: ## !!! HARD CODE here
            ...
            tt = context_img.shape[1] // 16
            if tt > 16:
                context_img = rearrange(context_img, 'b (t l) c -> b t l c', t=tt) # 2, t, 16, 1024]
                repeat_scale = math.ceil(t / tt)
                context_img = context_img.repeat_interleave(repeats=repeat_scale, dim=1) # [2*t, 16, 1024]
                context_img = context_img[:,:t,:,:]
                context_img = rearrange(context_img, 'b t l c -> (b t) l c', b=b, t=t) # 2*t, 16, 1024]
            else:
                context_img = rearrange(context_img, 'b (t l) c -> b t l c', t=tt) # 2, t, 16, 1024]
                context_img = context_img[:,:t, :,:]
                context_img = rearrange(context_img, 'b t l c -> (b t) l c', b=b, t=t) # 2*t, 16, 1024]
            ...
            context_text = context_text.repeat_interleave(repeats=t, dim=0) # [2*t, 77, 1024]
            # context_img = context_img.repeat_interleave(repeats=t, dim=0) # [2*t, 16, 1024]
            context = torch.cat([context_text, context_img], dim=1) # torch.Size([32, 93, 1024])
