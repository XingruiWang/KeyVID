...
class TheGreatestHits(Dataset):
    ...
    def _load_metadata(self):
        ...
        with open(self.caption_dir, 'r') as f: # xx.txt
            self.all_video_paths = f.readlines()
        self.all_video_paths = [p.strip() for p in self.all_video_paths]
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(keyframe_path):
            ...
            keypoint = np.zeros(48)
            keyframe = np.zeros(48)
            if self.keyframe_ratio > 0:
        ...
        if rand_num < self.keyframe_ratio:
            frame_indices_for_selection, frame_strides, frame_indices = self.select_keyframe(keypoint, start_idx, self.video_length, self.fixed_fps, fps_ori)
            frame_strides_fps = torch.tensor(np.array([int(fps_ori / max(fs, 1)) for fs in frame_strides]))
            fps_clip = int(frame_strides_fps.float().mean())
        ...
        data = {'video': frames, 'caption': caption, 'audio': waveform_melspec, 
                'path': video_path, 'fps': fps_clip, 'frame_stride': frame_stride, 'frame_strides': frame_strides, 'frame_indices': frame_indices, 'frame_strides_fps': frame_strides_fps,
...
if __name__== "__main__":
    ...
    data_dir = "/dockerx/local/data/TheGreatestHits/videos" ## path to the data directory
    keyframe_dir = "/dockerx/local/data/TheGreatestHits/train_curves_npy/" ## path to the keyframe directory 
    caption_dir = "/dockerx/local/data/TheGreatestHits/train.txt" ## path to the keyframe directory 

    L = 12
    dataset = TheGreatestHits(data_dir,
