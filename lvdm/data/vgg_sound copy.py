...
class VGGSound(Dataset):
    ...
    def _load_metadata_vgg(self, caption):
        ...
        for video_info in video_infos:
            ...
            if os.path.exists(os.path.join(self.data_dir, video_path)) and \
                os.path.exists(os.path.join(self.keyframe_dir, video_path.split(".")[0] + ".npy")) and \
                frame_num > 2.5 * fps \
                and fps >= 24:
    ...
    def select_frame(self, start_idx, target_length, target_fps=30, fps_ori=30, index=0):
        ...
        return random_keypoint.astype(int), frame_stride.astype(float), random_keypoint_condition.astype(int)
    ...
    def __getitem__(self, index):
        ...
        # print(f"frame_indices={frame_indices}, frame_strides={frame_strides}, frame_indices_for_selection={frame_indices_for_selection}")
        # import ipdb; ipdb.set_trace()
        # try:
        #     frame_indices, frame_strides = self.select_keyframe(keypoint, start_idx, self.video_length, self.fixed_fps, fps_ori)
        #     if len(frame_indices) != self.video_length:
        #         raise ValueError(f"frame_indices={len(frame_indices)}, self.video_length={self.video_length}")
        # except:
        #     print(f"keyframe_path={keyframe_path}, start_idx={start_idx}, video_length={self.video_length}, fixed_fps={self.fixed_fps}, fps_ori={fps_ori}")
        #     # import ipdb; ipdb.set_trace()
            
        # assert len(frame_indices) == self.video_length, f"frame_indices={len(frame_indices)}, self.video_length={self.video_length}"
        
        # end_idx = frame_indices[-1]
        try:
            frames = video_reader.get_batch(frame_indices_for_selection)
        except:
        ...
        video2, waveform, info = torchvision.io.read_video(video_path, pts_unit="sec") # [250, 720, 1280, 3])
        if waveform.size(0) == 1:
            waveform = waveform.expand(2, -1)
        sr = info["audio_fps"]

        print(len(video2), len(video_reader))
        all_frames = video_reader.get_batch(range(len(video_reader))).asnumpy()

        # frames_tmp = torch.tensor([video_reader[i] for i in range(len(video_reader))], dtype=torch.uint8)
        # save video

        torchvision.io.write_video("video_torchvision.mp4", video2, fps=fps_ori)
        torchvision.io.write_video("video_video_reader.mp4", all_frames, fps=fps_ori)
        print(f"sr={sr}, waveform={waveform.size()}, length={waveform.size(1) / sr}")
        start = Fraction(start_idx, frame_num) * frame_num / fps_ori
        end = start + 2.0
        print(f"start={start}, end={end}, start_idx={start_idx}, frame_num={frame_num}, fps_ori={fps_ori}, length={frame_num / fps_ori}")
        import ipdb; ipdb.set_trace()
...
if __name__== "__main__":
    ...
    # import os
    # import json
    # json.dump(metadata, open("metadata_vgg.json", "w"))
