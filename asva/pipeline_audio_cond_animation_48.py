...
def image_guided_synthesis(model, prompts, videos, audios, keyframes, frame_strides, \
    ...
    if model.use_qformer:
        ...
        cond["c_audio"] = model.split_audio(c_audio, 48)
    ...
    if unconditional_guidance_scale != 1.0 or multiple_cond_cfg:
        ...
        if model.use_qformer:
            ...
            uc_audio = model.split_audio(uc_audio, 48)
...
def save_video_batch(samples, audios, filenames, fakedir, fps=12, loop=False, idx = [0]):
    # import ipdb; ipdb.set_trace()
    print("Video frames: ", samples[0][0].shape[2], "Audio frames: ", audios[0].shape[1], "FPS: ", fps, "filenames: ", filenames)
