...
def load_data_batch(data_dir, filenames, video_size=(256,256), video_frames=16, video_fps=8, num_clips_per_video=3, fps_condition_type="kfs", swap_audio=False, batch_filename_neg=[]):
    ...
    for filename, filename_2 in zip(filenames, filenames_2):
        ...
        # predicted frame npy
        frame_npy = np.load(os.path.join('/dockerx/local/data/AVSync15/test_curves_npy', category), frame_npy_name)
        ...
        keyframe_save_path = '/dockerx/share/Dynamicrafter_audio/save/keyframe_idx_pred'
        ...
        frame_strides[:, 1:] = keyframes[:, 1:] - keyframes[:, :-1]
        ...
        if fps_condition_type == "kfs":
            ...
            # frame_strides_array =  np.array([max(1, int(24 / max(fs, 1))) for fs in frame_strides])
            frame_strides_array = []
            for frame_stride in frame_strides:
                frame_strides_array.append([max(1, int(24 / max(fs, 1))) for fs in frame_stride])
            frame_strides_array = np.array(frame_strides_array)
        ...
        frame_strides = torch.tensor(frame_strides_array).to(torch.float64)
