...
def load_data_batch(data_dir, filenames, video_size=(256,256), video_frames=16, video_fps=8, num_clips_per_video=3, fps_condition_type="kfs", swap_audio=False, batch_filename_neg=[]):
    ...
    for filename, filename_2 in zip(filenames, filenames_2):
        ...
        if swap_audio:
            filename_neg = filename_2
            load_videos_neg, load_audios_neg = load_av_clips_uniformly(
                os.path.join(data_dir, filename_neg), video_fps, video_frames, video_size, num_clips_per_video,
                load_audio_as_melspectrogram=False
            )
            load_audios = load_audios_neg
...
def image_guided_synthesis(model, prompts, videos, audios, keyframes, frame_strides, \
    ...
    c_audio = model.audio_proj_model(c_audio)
    c_audio = model.select_keyframes(c_audio, keyframes.to(img_emb).long(), num_queries=model.audio_proj_model.num_queries)
    c_audio = rearrange(c_audio, 'b (t l) c -> (b t) l c', l=model.audio_proj_model.num_queries) # 2*t, 8, 1024]

    cond["c_audio"] = c_audio
