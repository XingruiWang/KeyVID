...
# from lvdm.models.samplers.ddim_temporal_inpainting import DDIMSampler
...
def load_data_batch(data_dir, filenames, video_size=(256,256), video_frames=16, video_fps=8, num_clips_per_video=3, fps_condition_type="kfs", swap_audio=False, batch_filename_neg=[]):
    ...
    for filename, filename_2 in zip(filenames, filenames_2):
        ...
        # load keyframe video
        keyframe_gen_dir = "/dockerx/share/Dynamicrafter_audio/save/asva/asva_12_keyframe_add_idx/epoch=309-step=3720_audio_7.5_img_2.0/samples"
...
#         ipdb> keyframe_clips[0].shape
# torch.Size([12, 320, 512, 3])

        # keyframes = np.linspace(0, 48, num=12+1, dtype=int)[:-1]
        ...
        full_frame_idx  = torch.tensor(full_frame_idx_array).unsqueeze(0).repeat(num_clips_per_video, 1).to(torch.float64)
...
def image_guided_synthesis(model, prompts, videos, audios, keyframe_clips, keyframes, frame_strides, \
    ...
    # 
    assert model.model.conditioning_key == 'hybrid'
    if model.model.conditioning_key == 'hybrid':
        ...
        # keyframe 
        # z_keyframe = z[:,:,keyframes[0].long()].clone() # b c t h w ??
    ...
    if unconditional_guidance_scale != 1.0 or multiple_cond_cfg:
        if model.uncond_type == "empty_seq":
            u_prompts = batch_size * [""]
            uc_emb = model.get_learned_conditioning(u_prompts)
    ...
    for _ in range(n_samples):
        ...
        if ddim_sampler is not None:
            ...
            if intermediates is not None:
                inter_decode = []
                for step_idx, inter in enumerate(intermediates['x_inter']):
                    inter = model.decode_first_stage(inter)

                    inter_decode.append(inter) 
               
                
                    inter = inter.permute(0, 2, 3, 4, 1)[0]

                

                    video = torch.clamp(inter.float(), -1., 1.)
                    video = (video + 1.0) / 2.0
                    video = (video * 255).to(torch.uint8).cpu()

                    save_path = f"save/inter/{prompts[0].replace(' ', '_')}/step-{step_idx}.mp4"
                    os.makedirs(os.path.dirname(save_path), exist_ok=True)
                    torchvision.io.write_video(save_path, video, fps=24, video_codec='h264', options={'crf': '10'})
                    print(f"intermediate video saved at {save_path}")
