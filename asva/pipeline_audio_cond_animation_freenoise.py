...
def load_data_batch(data_dir, filenames, video_size=(256,256), video_frames=16, video_fps=8, num_clips_per_video=3, fps_condition_type="kfs", swap_audio=False, batch_filename_neg=[]):
    ...
    for filename, filename_2 in zip(filenames, filenames_2):
        ...
        # 30 frames
        keyframe_array = np.linspace(0, 48, num=video_frames+1, dtype=int)[:-1]
        # keyframes = torch.tensor(np.linspace(0, 48, num=video_frames+1, dtype=int)[:-1]).repeat(num_clips_per_video, 1).to(torch.float64)
        ...
        elif fps_condition_type == "fps":
            frame_strides_array = np.array(keyframe_array)
        elif fps_condition_type =="kfs2fps":
            # frame_strides_array = np.array([6]*12)
            frame_strides_array = np.array([6]*48)
...
def image_guided_synthesis(model, prompts, videos, audios, keyframes, frame_strides, \
    ...
    frame_idx = keyframes.to(img_emb).long()
    ...
    # x_T_total = torch.randn(noise_shape, device=model.device)[None, ...]
    # for frame_index in range(12, 48, 4):
    #     list_index = list(range(frame_index-12, frame_index+4-12))
